...
const editPost = async (req, res) => {
    try {
        ...
        res.status(200).json({ error: 'This publication id does not exist' })
    }
}
...
const editUser = async (req, res) => {
    try {
        ...
        await UserSchema.update({ username, email, password, verified, role }, { where: { id } })
        res.status(200).json({ message: 'User has been edited' })
    } catch (e) {
        res.status(200).json({ error: 'This user id does not exist' })
    }
}
